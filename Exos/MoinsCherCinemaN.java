...
public class MoinsCherCinemaN { //com.mysql.cj.jdbc.Driver
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (rs.getInt(1) < moinscher)
                {
                    count = 1;
                    moinscher = rs.getInt(1);
                }
                else if (rs.getInt(1) == moinscher)
                    count ++;

            }

            String Affichage = "SELECT nomC FROM cinema WHERE tarifnormal =" + moinscher ;
            ...
            if (count == 1)
                System.out.println("\n Le cinema le moins chere en tarif normal est ");
            else
                System.out.println("Les cinemas les moins cheres en tarif normal sont ");

            // ______________ AFFICHAGE DES DONNEES ____________________________
            System.out.println("\n | \t Cinema \t Tarif ");
            System.out.println(" | \t ------- \t ------- ");
            while (rs2.next()) {
                System.out.println(" | \t" + rs2.getString(1) + "\t" + moinscher + " Fcfa"); //6 : Traitement du résultat
            }
